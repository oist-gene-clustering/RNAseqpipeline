...
#THIRD IMPORTANT NOTE: From https://github.com/griffithlab/rnaseq_tutorial/wiki/Annotation
...
#(can be easily extended to several-file processing though: see script_multiple.sh)

#THIS SCRIPT IS FOR ANALYSIS OF BULK (REGULAR) RNA-SEQ RESULTS
#see script-sc.py for Single-Cell RNA sequencing

#This one reference is quite useful:
...
kmer = 31
#FRAGLENMEAN: Average fragment length (unpaired/SE reads only) should be the same as for Cufflinks...
fragLenMeanK = 200
#FRAGLENSTDDEV: Fragment length std deviation (unpaired/SE reads only) should be the same as for Cufflinks...
fragLenStdDevK = 80
#THREADSK: Number of threads
threadsk = 8
...
def referenceFromData(filename, mode, clean = False):
    ...
    cmd = kallistoFolder + "kallisto quant -l " + fragLenMeanK + " -s " + fragLenStdDevK + " -t " + str(threadsk) 
    cmd += " -i " + indexFolder + filename + "_kindex -o " + kallFolder + " --plaintext "
    if bias:
        cmd += "--bias "
    if single:
        cmd += "--single "
...
# DE-COMMENT NON-COMMENT LINES TO GET THE WHOLE PIPELINE
...
def main(filename, mode, trim):
    ...
    ### POST-ALIGNMENT QUALITY CONTROL (with FASTQC)
    #qualityControlSplit(filename, "bam", "SE")
    ...
    #do = False So you can see the files for each step
    #cleanFiles(filename, fastqcName, False)
    
#######################################################################################################################################

######### DEBUGGING LINE, SHOULD BE DISMISSED to test for other files
main("homosap", "PE", "N")
######### to use the pipeline: open python shell, then call function "main" with the good arguments